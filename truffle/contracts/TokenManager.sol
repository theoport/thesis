...
contract TokenManager {
  ...
  /*  Variables that are internal to contract. */
  ...
  event Debug(uint256 d, uint256 c, uint256 b, uint256 a);  
  ...
  event NewVote(uint256[3] tag, uint256 yes, uint256 no, address from);

  event BugHuntStarted(uint256 time, uint256 finishTime, uint256 updateId, uint256 bugHuntNumber, uint256 creationTime, address updatedContract, uint256 safetyHash);
  event BugHuntEnd(uint256 time, uint256 updateId, uint256 bugHuntNumber);
  event BugFound(uint256 time, uint256 indexed updateId, uint256 indexed bugId, address by, uint256 descriptionHash);
  event WasABug(uint256 updateId, uint256 bugHuntNumber, uint256 bugId, uint256 finishTime, address developer, uint256 tries); 
  event WasNotABug(uint256 updateId, uint256 bugHuntNumber, uint256 bugId);
  ...
  function setBugHunt (uint256 _hunt) byCreator {BUGHUNT = _hunt * (1 minutes);}
  //function setBugHunt (uint256 _hunt) byCreator {BUGHUNT = _hunt;}
  function setChangeOverTime (uint256 _time) byCreator {CHANGEOVERTIME = _time * (1 minutes);}
  //function setChangeOverTime (uint256 _time) byCreator {CHANGEOVERTIME = _time;}
  function setBugExtension (uint256 _extension) byCreator {BUGEXTENSION = _extension * (1 minutes);}
  //function setBugExtension (uint256 _extension) byCreator {BUGEXTENSION = _extension;}
  function setBountyHunt(uint256 _huntDuration) byCreator {BOUNTYHUNT = _huntDuration * (1 minutes);}
  ...
  function setVoteDuration(uint256 _duration) public byCreator {VOTEDURATION = _duration * (1 minutes);}
  //function setVoteDuration(uint256 _duration) public byCreator {VOTEDURATION = _duration;}
  function setAuctionDuration(uint256 _duration) public byCreator {AUCTIONDURATION = _duration * (1 minutes);}
  ...
  function startBounty (uint256 _id, uint256 _safetyHash) public {
    ...
    require (!changeOverActive && !bountyHunt.active && !update.active);

    bountyHunt.active = true;
    bountyHunt.startTime = now;
    bountyHunt.endTime = bountyHunt.startTime + BOUNTYHUNT;
    bountyHunt.bestPrice = 0;
    update.id= _id;
    update.safetyHash = _safetyHash;

    UpdateStarted(bountyHunt.startTime, update.id);
    BountyStarted(update.id, bountyHunt.startTime, bountyHunt.endTime, _safetyHash);
  }
  ...
  function bidForBounty (uint256 price) public {
    ...
    require((now >= bountyHunt.startTime && now <= bountyHunt.endTime)); 

    if (price < bountyHunt.bestPrice || bountyHunt.bestPrice == 0) {
      bountyHunt.bestPrice = price;
      bountyHunt.bidder = msg.sender;
  ...
  function endBounty () public {
    ...
    require(bountyHunt.active && now >= bountyHunt.endTime);

    bountyHunt.active = false;
    BountyEnd(update.id, now, bountyHunt.bidder, bountyHunt.bestPrice);

    if (bountyHunt.bestPrice == 0){
      UpdateOutcome(now, false, update.id);
    } else {
      voteOnUpdate(bountyHunt.bidder, bountyHunt.bestPrice);
    }
  }
  ...
  function foundBug (uint256 _descriptionHash, uint256 _id) public {
    ...
    update.bugHunt.bug.descriptionHash = _descriptionHash;
    update.bugHunt.bug.finder = msg.sender;
    update.endTime += VOTEDURATION; 
    update.bugHunt.bug.bugId = _id;
    BugFound(now, update.id, update.bugHunt.bug.bugId, msg.sender, _descriptionHash);
    startVote([uint256(subject.BUG), update.id, update.bugHunt.bug.bugId]);
  ...
  function voteOnUpdate (address _developer, uint256 _price) internal {
    ...
    update.price = _price;
    update.timeWindow = (_price / PRICEHOURRATIO) * (1 minutes);
  ...
  function submitUpdate (address _updatedContract) public{
    ...
    BugHuntStarted(update.bugHunt.startTime, update.bugHunt.endTime, update.id, update.bugHunt.bugHuntNumber, newToken.creationTime(), update.updatedContract, update.safetyHash);
  ...
  function finaliseUpdate() public {
    ...
    if (!token.transfer(update.developer, update.price)){
      token.add(update.developer, update.price);  
    }
    
    update.active = false;
    update.bugHunt.active = false;
    changeOverActive = true;
    newToken.initialise(token.creator(), token.name(), token.getIssuanceRate(), token.upperCap(), token.limitReached(), token.lastUpdate(), token.noAccountsBlocked(), token.totalSupply(), token.daysSinceFirstCreation(), token.creationTime());  
    version++;
    ...
    BugHuntEnd(now, update.id, update.bugHunt.bugHuntNumber);
  ...
  function startVote(uint256[3] _tag) internal {
    ...
    VoteStarted(vote.startTime, vote.endTime, vote.tag); 
  }
  ...
  function endVote() public returns (bool){
    ...
    if (vote.totalPossibleVotes > 0 && ((vote.totalYesVotes * 100) / vote.totalPossibleVotes) >= CONSENSUSPERCENT){
      ...
      if (uint256(vote.tag[0]) == uint256(subject.BUG)) {
        ...
        update.bugHunt.active = false;
        token.transfer(update.bugHunt.bug.finder, (update.price / (100 / update.bugBounty)));
        ...
        update.hasSubmitted = false;
        BugHuntEnd(now, update.id, update.bugHunt.bugHuntNumber);
        WasABug(update.id, update.bugHunt.bugHuntNumber, update.bugHunt.bug.bugId, update.endTime, update.developer, update.numberOfTries);
        return true;
      ...
      if (uint256(vote.tag[0]) == uint256(subject.UPDATE)) {
        ...
        update.bugHunt.foundOne = false;
        WasNotABug(update.id, update.bugHunt.bugHuntNumber, update.bugHunt.bug.bugId);
        return true;
      }
