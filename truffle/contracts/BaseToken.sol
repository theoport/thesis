...
contract BaseToken is ERC20Standard{
	...
	/*Lets the creator mint tokens from the tokenmanager contract.*/
	...
	function initialise(address _creator, bytes32 _name, uint256[2] _issuanceRate, uint256 _upperCap, bool _limitReached, uint256 _lastUpdate, uint256 _noAccountsBlocked, uint256 _totalSupply, uint256 _daysSinceFirstCreationLast, uint256 _creationTimeLast) byManager {
		...
		daysSinceFirstCreation = creationTime - _creationTimeLast + _daysSinceFirstCreationLast;
	}
		...
		if(!limitReached) {
			...
			if (daysPassed > lastUpdate) {
				...
				for (uint256 i = lastUpdate; i < daysPassed; i++) {
				 		...
				 		if ((issuanceRate[1] * i + issuanceRate[0]) > 0){
